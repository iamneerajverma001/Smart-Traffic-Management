...
scenario_1_frames = [
    ...
    [{'box': [100, 150, 300, 400], 'score': 0.95, 'class': 'car'},
     {'box': [450, 200, 700, 500], 'score': 0.88, 'class': 'ambulance'},
     {'box': [50, 400, 250, 550], 'score': 0.91, 'class': 'ambulance'}],
    # Frame 1
    [{'box': [120, 160, 320, 410], 'score': 0.94, 'class': 'car'},
     {'box': [470, 210, 720, 510], 'score': 0.87, 'class': 'ambulance'},
     {'box': [60, 410, 260, 560], 'score': 0.90, 'class': 'ambulance'},
...
# Add 'ambulance' to the list of vehicle classes
...
def process_video(video_source):
    ...
    while cap.isOpened():
        ...
        for det in detections:
            ...
            # Ensure only vehicle classes are processed
            if class_name not in vehicle_classes:
                continue

            # Overwrite detected class to 'ambulance' for bus and truck
            if class_name in ['bus', 'truck']:
                class_name = 'ambulance'

            # Ensure counts for bus and truck are added to 'ambulance'
            if class_name in ['bus', 'truck']:
                frame_vehicle_counts['ambulance'] += 1
            else:
                frame_vehicle_counts[class_name] += 1

            # Draw bounding box and label
            color = (0, 255, 0) if class_name == 'ambulance' else (255, 0, 0)
        ...
        # Flash full screen alert for emergency vehicles
